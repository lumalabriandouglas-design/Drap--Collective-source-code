...
function ProductPage() {
  ...
  const productQuery = useQuery({ queryKey: ["product", slug], queryFn: () => getProduct({ data: slug }) });
  const product = productQuery.data;
  const related = useQuery({
    queryKey: ["related", slug],
    enabled: Boolean(product),
    queryFn: () => listRelated({ data: { slug, category: product!.category, designerSlug: product!.designer.slug } }),
  });
  ...
  const selectedSize = useMemo(() => size || (product?.sizes.length === 1 ? product.sizes[0] : null), [size, product]);
  ...
  function addToBag() {
    if (!selectedSize) { toast("Select a size to continue"); return; }
    add({ productId: piece.id, slug: piece.slug, name: piece.name, designerName: piece.designer.name, image: images[0], priceCents: piece.priceCents, size: selectedSize });
  ...
  async function onEnquire() {
    if (!user) { void navigate({ to: "/login" }); return; }
    try {
      await sendInquiry({ data: { productId: piece.id, designerId: piece.designer.id, message: note || `I would like to enquire about ${piece.name}.` } });
      setNote("");
      toast.success("Note sent to the atelier");
    } catch (err) {
    ...
    <main className="mx-auto max-w-7xl px-4 pt-24 pb-20 sm:px-6 lg:px-8 lg:pt-28">
      <div className="grid items-start gap-10 lg:grid-cols-2 lg:gap-16">
        <div>
          ...
          {images.length > 1 && (
            <div className="mt-3 flex gap-2">
              {images.map((src, i) => (
                <button key={src} type="button" onClick={() => setActive(i)} className={`h-20 w-16 overflow-hidden rounded-lg border ${i === active ? "border-charcoal-800" : "border-transparent"}`}>
        ...
        <div>
          ...
          {product.leadTime && <p className="mt-4 text-xs uppercase tracking-[0.12em] text-charcoal-400">{product.leadTime}</p>}
          <div className="mt-8">
            <p className="text-[10px] font-medium uppercase tracking-[0.16em] text-gold-600">Size</p>
            <div className="mt-3 flex flex-wrap gap-2">
              {product.sizes.map((option) => (
                <button key={option} type="button" onClick={() => setSize(option)} className={`h-11 min-w-11 rounded-full px-4 text-xs tracking-[0.08em] transition-colors ${selectedSize === option ? "bg-charcoal-800 text-ivory-50" : "border border-border bg-ivory-50 text-charcoal-600 hover:border-charcoal-400"}`}>
                  {option}
                </button>
              ))}
            </div>
          ...
          {product.materials.length > 0 && (
            <p className="mt-6 text-sm text-charcoal-500"><span className="text-charcoal-400">Materials · </span>{product.materials.join(", ")}</p>
          )}
          <div className="mt-8 flex flex-wrap gap-3">
            <Button size="lg" onClick={addToBag}>Add to bag</Button>
            <Button size="lg" variant="outline" onClick={() => void onSave()}><Heart size={15} fill={saved ? "currentColor" : "none"} /> Save</Button>
          </div>
          <div className="mt-10 rounded-2xl border border-border bg-ivory-50 p-5">
            <p className="flex items-center gap-2 text-[10px] font-medium uppercase tracking-[0.16em] text-gold-600"><MessageCircle size={14} /> Enquire with the atelier</p>
            <Textarea className="mt-3 bg-white" value={note} onChange={(e) => setNote(e.target.value)} placeholder="A commission, a size question, a private viewing…" />
            <Button className="mt-3" variant="outline" onClick={() => void onEnquire()}>Send note</Button>
          </div>
        </div>
      </div>
