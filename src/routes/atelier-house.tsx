...
import { loadVisitStats, subscribeVisits } from "@/lib/visits";
...
function AtelierHouse() {
  ...
  const visits = useQuery({
    ...
    enabled: isAdmin,
    queryFn: () => loadVisitStats(),
    refetchInterval: 12_000,
  });

  useEffect(() => {
    if (!isAdmin) return;
    return subscribeVisits(() => {
      void visits.refetch();
    });
  }, [isAdmin, visits.refetch]);
  ...
  const designers = profiles.filter((p) => p.role === "designer" || p.brand_name).length;

  return (
    <HouseRoom
      eyebrow="Admin"
      title="The book"
      lede="Who is here, and how many people opened the site. The visit number grows on its own."
      actions={
        <>
    ...
    >
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <RoomStat label="Visits" value={visits.data?.total ?? "—"} />
        <RoomStat label="Visits today" value={visits.data?.today ?? "—"} />
        <RoomStat label="Designers" value={designers} />
        <RoomStat label="People" value={profiles.length} />
      </div>
      ...
      <section className="mt-14">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-[10px] font-medium uppercase tracking-[0.22em] text-gold-600">People</p>
            <h2 className="mt-2 font-serif text-3xl text-charcoal-800">Accounts</h2>
          </div>
        ...
        {visible.length === 0 ? (
          <div className="mt-8">
            <RoomEmpty title="No match" body="Try another name, or clear the filter." />
          </div>
        ) : (
          <ul className="mt-8 divide-y divide-charcoal-100 rounded-2xl border border-charcoal-100 bg-ivory-50">
            {visible.map((profile) => {
              const name = profile.brand_name?.trim() || profile.username?.trim() || "Member";
              return (
                <li
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-xs text-charcoal-400">{profile.email ?? "No email"}</p>
                  </div>
