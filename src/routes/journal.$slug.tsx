  ...
  if (!query.data) {
    return (
      <main className="mx-auto max-w-xl px-4 pt-32 pb-24 text-center">
        ...
        <Button asChild className="mt-8"><Link to="/journal">Journal</Link></Button>
      </main>
    );
  }
  ...
  return (
    <main>
      <section className="relative min-h-[60vh] overflow-hidden bg-charcoal-900">
        <img src={lookbook.coverUrl} alt={lookbook.title} className="absolute inset-0 size-full object-cover" />
        ...
        <div className="relative mx-auto flex min-h-[60vh] max-w-4xl flex-col justify-end px-4 pb-12 pt-32">
          <p className="text-[11px] uppercase tracking-[0.22em] text-gold-300">{lookbook.designerName ?? "Journal"}</p>
          <h1 className="mt-3 font-serif text-5xl text-ivory-50">{lookbook.title}</h1>
          {lookbook.subtitle && (
            <p className="mt-3 font-serif text-xl italic text-ivory-100/80">{lookbook.subtitle}</p>
          )}
        </div>
      </section>
      <article className="mx-auto max-w-2xl px-4 py-16">
        {paragraphs.map((p) => (
          <p key={p.slice(0, 24)} className="mb-6 text-base font-light leading-relaxed text-charcoal-600">{p}</p>
        ))}
      </article>
      {pieces.length > 0 && (
