...
function Ateliers() {
  ...
  const designers = (query.data ?? []).filter(
    (atelier) => atelier.pieceCount > 0 || !/^independent designer$/i.test(atelier.name),
  );
        ...
        {designers.map((atelier) => (
          ...
          >
            ...
            <p className="text-xs uppercase tracking-[0.14em] text-charcoal-400">
              ...
              {atelier.pieceCount > 0
                ? `${atelier.pieceCount} ${atelier.pieceCount === 1 ? "piece" : "pieces"}`
                : "Showroom open"}
            </p>
