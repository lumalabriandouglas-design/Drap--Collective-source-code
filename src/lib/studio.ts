...
export const getMyStudio = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const pieces = await sql.query<{
      ...
      price_cents: number; materials: unknown; sizes: unknown; image_urls: unknown;
      tags: unknown; lead_time: string | null; featured: boolean; listed_by: string | null;
    }>(
      `select id, slug, name, description, category, price_cents, materials, sizes, image_urls, tags, lead_time, featured, listed_by
       from products where designer_id = $1 order by created_at desc`,
      [atelier.id],
    );

    return {
      atelier: { id: Number(atelier.id), slug: atelier.slug, name: atelier.name, city: atelier.city, country: atelier.country, bio: atelier.bio },
      pieces: pieces.map((row) => ({
        ...
        designer: { id: Number(atelier.id), slug: atelier.slug, name: atelier.name, city: atelier.city, country: atelier.country, imageUrl: "" },
      })),
    };
  });

export const openAtelier = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  ...
  .handler(async ({ context, data }) => {
    ...
    const sql = await getSql();
    const existing = await sql.query<{ id: number }>(`select id from designers where user_id = $1`, [context.userId]);
    if (existing.length) {
      return { slug: (await sql.query<{ slug: string }>(`select slug from designers where user_id = $1`, [context.userId]))[0].slug };
    }
    ...
    await sql.query(
      ...
      [slug, name, city, country, bio, null, "/images/products/studio-2.jpg", context.userId],
    );
    await sql.query(
      `insert into profiles (user_id, display_name, role) values ($1, $2, 'designer')
...
export const listPiece = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  .validator((input: { name: string; description: string; category: string; price: number; sizes: string[]; imageUrl: string; leadTime: string }) => input)
  .handler(async ({ context, data }) => {
    const name = data.name.trim();
    const description = data.description.trim();
    if (name.length < 2) throw new Error("Name the piece.");
    if (description.length < 12) throw new Error("Add a short atelier note.");
    if (!PRODUCT_CATEGORIES.includes(data.category as (typeof PRODUCT_CATEGORIES)[number])) throw new Error("Choose a category.");
    const priceCents = Math.round(Number(data.price));
    if (!Number.isFinite(priceCents) || priceCents < 5000) throw new Error("Set a price of at least USh 5,000.");
    const sizes = data.sizes.filter((size) => PRODUCT_SIZES.includes(size as (typeof PRODUCT_SIZES)[number]));
    if (!sizes.length) throw new Error("Select at least one size.");

    const sql = await getSql();
    const ateliers = await sql.query<{ id: number; slug: string; name: string }>(`select id, slug, name from designers where user_id = $1 limit 1`, [context.userId]);
    const atelier = ateliers[0];
    if (!atelier) throw new Error("Open an atelier before listing a piece.");

    const slug = slugify(name);
    const imageUrl = data.imageUrl || "/images/products/studio-2.jpg";
    await sql.query(
      `insert into products (slug, designer_id, name, description, category, price_cents, materials, sizes, image_urls, tags, lead_time, featured, listed_by)
       values ($1,$2,$3,$4,$5,$6,$7::jsonb,$8::jsonb,$9::jsonb,$10::jsonb,$11,false,$12)`,
      [slug, atelier.id, name, description, data.category, priceCents, JSON.stringify([]), JSON.stringify(sizes), JSON.stringify([imageUrl]), JSON.stringify(["atelier"]), data.leadTime.trim() || "Made to order · 3 weeks", context.userId],
    );
    invalidateFloor();
    return { slug };
  });

export const storageStatus = createServerFn({ method: "GET" }).handler(async () => {
  const { r2Status } = await import("@/lib/r2");
  return r2Status();
});

export const uploadPiecePhoto = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  ...
  .handler(async ({ context, data }) => {
    ...
    return storeImage({ filename: data.filename || "piece.webp", mime, bytes, folder: `pieces/${context.userId}` });
  });
