...
export function DesignerShowroom({ designer, pieces }: { designer: Designer; pieces: Product[] }) {
  ...
  async function share() {
    const payload = { title: `${designer.name} — Drapé Collective`, text: `The ${designer.name} showroom on Drapé Collective.`, url: shareUrl };
    try {
      ...
      <main>
        <section className="relative min-h-[88vh] overflow-hidden bg-charcoal-900">
          ...
          <div className="relative mx-auto flex min-h-[88vh] max-w-7xl flex-col justify-end px-4 pb-12 pt-28 sm:px-6 lg:px-8 lg:pb-16">
            <p className="flex items-center gap-2 text-[11px] uppercase tracking-[0.22em] text-gold-300">
              <MapPin size={12} /> {designer.city}, {designer.country}
            </p>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              {hero && (
                <Button asChild size="lg" variant="outline" className="border-ivory-50/50 text-ivory-50 hover:bg-ivory-50 hover:text-charcoal-800">
                  <Link to="/shop/$slug" params={{ slug: hero.slug }}>View this piece</Link>
                </Button>
              )}
            </div>
          </div>
        ...
        <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
          <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
            <div>
              <p className="text-[10px] font-medium uppercase tracking-[0.22em] text-gold-600">Private showroom</p>
          ...
          {pieces.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-8 md:grid-cols-12">
              {pieces.map((piece, i) => {
                const featured = i === 0;
                return (
                  <Link key={piece.slug} to="/shop/$slug" params={{ slug: piece.slug }} onMouseEnter={() => setActive(i)} className={cn("group block", featured ? "md:col-span-7" : "md:col-span-5")}>
                    <LazyImage src={piece.imageUrls[0] ?? designer.imageUrl} alt={piece.name} width={featured ? 1440 : 900} eager={i < 2} className={cn("rounded-2xl", featured ? "aspect-portrait md:min-h-[32rem]" : "aspect-portrait")} imgClassName="transition-transform duration-700 group-hover:scale-[1.03]" />
                    <div className="mt-4 flex items-start justify-between gap-4">
                      <div>
                        <h3 className="font-serif text-2xl text-charcoal-800">{piece.name}</h3>
                        <p className="mt-1 text-xs uppercase tracking-[0.12em] text-charcoal-400">{piece.category}{piece.leadTime ? ` · ${piece.leadTime}` : ""}</p>
                      </div>
